...
if not firebase_admin._apps:
    if service_account_info:
        # Production: Load credentials from Render Environment Variable
        cred_dict = json.loads(service_account_info)
        cred = credentials.Certificate(cred_dict)
    else:
        # Local: Fallback to the local serviceAccount.json file
        try:
            cred = credentials.Certificate("serviceAccount.json")
        except FileNotFoundError:
            raise FileNotFoundError("Firebase credentials not found. Add serviceAccount.json or set FIREBASE_SERVICE_ACCOUNT_JSON.")
...
# ---------- Flask app ----------
app = Flask(__name__)
# Use an environment variable for the secret key, or a default for local dev
...
def get_items_by_type(item_type=None, search_query="", category=None, limit=None):
    """Fetches items from Firestore with optional filtering."""
...
def report_item(item_type):
    ...
    # FIX: Pass today's date to fix Jinja2 TemplateAssertionError
    ...
    if request.method == "POST":
        ...
        # Handle Cloud Image Upload to Firebase Storage
        image_url = ""
...
if __name__ == "__main__":
    # Render binds to a specific port; local development uses 5000
